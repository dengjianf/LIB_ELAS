...
void Elas::process (uint8_t* I1_,uint8_t* I2_,float* D1,float* D2,const int32_t* dims){
 //////////////////////////内存对齐 ////////////////////////////////////////////////////
  ...
  width = dims[0];//左图列数
  height = dims[1];//左图行数
  bpl    = width + 15-(width-1)%16;    //每一行的字节
  
  // copy images to byte aligned memory//字节对齐存储器
...
#endif
/*
描述子通常是一个向量，按照人为的设计的方式，
描述关键点周围像素的信息。
通常描述子是按照外观相似的特征应该有相似的描述子设计的。
因此，在匹配的时候，只要两个特征点的描述子在向量空间的距离相近，
就可以认为它们是同一个特征点。
*/
  Descriptor desc1(I1,width,height,bpl,param.subsampling);
  Descriptor desc2(I2,width,height,bpl,param.subsampling);//subsample 二次采样
...
inline int16_t Elas::computeMatchingDisparity (const int32_t &u,const int32_t &v,uint8_t* I1_desc,uint8_t* I2_desc,const bool &right_image) {
  ...
  int32_t desc_offset_1 = -16*u_step-16*width*v_step;//offset 地址校正  block的四个角点的地址相对中心的偏移
  ...
  __m128i xmm1,xmm2,xmm3,xmm4,xmm5,xmm6;// 这是寄存器。

  // check if we are inside the image region   3×3的窗口扫描图像
  if (u>=window_size+u_step && u<=width-window_size-1-u_step && v>=window_size+v_step && v<=height-window_size-1-v_step) {
    
    // compute desc and start addresses  计算描述子和开始地址
    int32_t  line_offset = 16*width*v;      //block所在行的起始地址的偏移
    uint8_t *I1_line_addr,*I2_line_addr;
    if (!right_image) {//左图
      I1_line_addr = I1_desc+line_offset;       //行起始地址
    ...
    uint8_t* I1_block_addr = I1_line_addr+16*u;   //  块 起始地址
    uint8_t* I2_block_addr;
    
    // we require at least some texture//texture 纹理
    int32_t sum = 0;
    for (int32_t i=0; i<16; i++)
      sum += abs((int32_t)(*(I1_block_addr+i))-128);//sum of abs difference   理解：这个块里面的sobel梯度必须有一定的差异，即纹理
    if (sum<param.support_texture)                  //为啥是减128啊。。。。
      return -1;
    
    // load first blocks to xmm registers
	/*
	     _mm_load_si128() 将指针所指地址的数据加载到寄存器中
	*/
    ...
    // declare match energy for each disparity   计算每个视差的匹配代价并挑选出最小的那一个
    int32_t u_warp;
    ...
    int32_t disp_min_valid = max(param.disp_min,0);//disp下限必须大于零
    ...
    // assume, that we can compute at least 10 disparities for this pixel 视差范围至少10个等级
    ...
    for (int16_t d=disp_min_valid; d<=disp_max_valid; d++) {
      ...
      I2_block_addr = I2_line_addr+16*u_warp;  //   右图匹配

      // compute match energy at this disparity
      xmm6 = _mm_load_si128((__m128i*)(I2_block_addr+desc_offset_1));
      xmm6 = _mm_sad_epu8(xmm1,xmm6);//计算sad 
	  // sum of adsolute different 
	  /*参考 https://docs.microsoft.com/en-us/dotnet/api/system.runtime.intrinsics.x86.sse2.sumabsolutedifferences?view=netcore-3.1 */
      ...
      // best + second best match   得到最小和第二小的匹配代价（匹配能量）
      if (sum<min_1_E) {
        min_2_E = min_1_E;   
        min_2_d = min_1_d;
        min_1_E = sum;
...
vector<Elas::support_pt> Elas::computeSupportMatches (uint8_t* I1_desc,uint8_t* I2_desc) {
  ...
  // from every second line!         ？？？？什么意思
  int32_t D_candidate_stepsize = param.candidate_stepsize;//支持点匹配的规则网格步长
  if (param.subsampling)
    D_candidate_stepsize += D_candidate_stepsize%2;
  // create matrix for saving disparity candidates       计算步长
  ...
  /*
  删除直线上的支撑点，应为他们是多余的，这可以减少三角剖分的时间
  */
...
void Elas::computeDisparityPlanes (vector<support_pt> p_support,vector<triangle> &tri,int32_t right_image) {
  ...
  for (int32_t i=0; i<tri.size(); i++) {
    ...
    // get triangle corner indices  角的引索
    int32_t c1 = tri[i].c1;
    int32_t c2 = tri[i].c2;
    int32_t c3 = tri[i].c3;
