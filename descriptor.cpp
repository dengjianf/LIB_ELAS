...
Descriptor::Descriptor(uint8_t* I,int32_t width,int32_t height,int32_t bpl,bool half_resolution) {
  ...
  filter::sobel3x3(I,I_du,I_dv,bpl,height);//Sobel 求导
...
//////////////////////////////////一般情况未使用///////////////////////////////////////////////////////
  ...
  // compute full descriptor images  默认情况
  } else {
    
    // create filter strip//        sobel滤波
    for (int32_t v=3; v<height-3; v++) {
      ...
      for (int32_t u=3; u<width-3; u++) {//1+11+5=17维的描述子
        I_desc_curr = I_desc+(v*width+u)*16;
